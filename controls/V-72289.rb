control "xccdf_mil.disa.stig_rule_SV-86913r3_rule" do
  ...
  impact 0.5
  describe.one do
    ipv4_default_accept_redirects = file("/etc/sysctl.conf").content.to_s.scan(/^\s*net\.ipv4\.conf\.default\.accept_redirects[\s]*=[\s]*(\d+)\s*$/).flatten
    describe ipv4_default_accept_redirects.reject { |x| x == "0" } do
      it { should be_empty }
    end
    files = command("find /etc/sysctl.d -type f -regex .\\*/\\^.\\*\\\\.conf\\$").stdout.split
    describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content !~ /^\s*net\.ipv4\.conf\.default\.accept_redirects[\s]*=[\s]*(\d+)\s*$/ } do
