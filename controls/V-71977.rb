control "xccdf_mil.disa.stig_rule_SV-86601r2_rule" do
  ...
  describe.one do
    describe file("/etc/yum.conf") do
      its("content") { should match(/^\s*gpgcheck\s*=\s*(\S+)\s*$/) }
    end
    file("/etc/yum.conf").content.to_s.scan(/^\s*gpgcheck\s*=\s*(\S+)\s*$/).flatten.each do |entry|
      describe entry do
        it { should eq "1" }
      end
    end
